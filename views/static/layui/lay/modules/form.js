			...
			this.config = {
				verify: {
					required: function (value, dom) {
						var reg = /[\S]+/;
						if (!reg.test(value)) {
							var titleDom=document.getElementById(dom.name)
							var title=titleDom.innerText.replace(":","/")
							//console.log(title.innerText)
							return "请输入"+title.trim();//获得元素的NAME
							//var index = dom.getAttribute("data-index");//获得data-index元素
							//return dom.name'不能为空';
							}
					},
//					required: [/[\S]+/, "必填项不能为空"],
					select:function(value, dom){
						//console.log(document.getElementsByName(dom.name)[0])
						//console.log(value)
						if (value==null) {
							var select=document.getElementById(dom.name)
							return "请选择"+select.innerText;
						}
					},
					isCheck:function(value, dom){
						var checkBox=document.getElementsByName(dom.name)
						var checkVal=false;
...
//						var checkVal=$("input[name='"+dom.name+"']").attr("checked", true);
//						console.log(checkVal)
						if (checkVal==false) {
							var title=document.getElementById(dom.name)
							return "请选择"+title.innerText;
						}
//						return dom.name;
					},
					isRadio:function(value, dom){
						...
						if (radioVal==false) {
							var isRadio=document.getElementById(dom.name)
							return "请选择"+isRadio.innerText;
						}
					},
